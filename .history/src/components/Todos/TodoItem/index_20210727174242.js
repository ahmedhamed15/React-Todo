...
const Index = ({ task, setItemState, deleteItem, updateItem, fullState }) => {
    ...
    const editHandle = (e) => {
        console.log(fullState.editError);
        e.preventDefault();
        changeEditState();
            ...
            <label className={ !editState.isEdit ? "todo" : "todo edit-mode"}>
                { !editState.isEdit ? (
                    ...
                    <form className="todo-form edit-form" onSubmit={editHandle}>
                        ...
                        {fullState.editError !== '' && (<div className="error-message"> {fullState.editError} </div>)}
                    </form>
                )}
