    ...
    return (
        <section className="todos-list">
            ...
            <div className="todo-list">
                ...
                { state.tasks.length > 0 ? state.tasks.map( (item) => <TodoItem key={item.id} task={item} fullState={state} editError={state.editError} setItemState={setItemState} deleteItem={deleteItem} updateItem={updateItem} /> ) : (<div className="no-result"> There are no items, yet! </div>) }
            </div>
