...
const Index = () => {
    ...
    const setItemState = (id) => {
        let curItems = [...state.tasks],
            itemItSelf = curItems.find( (item) => item.id === id {item.active = !item.active}),
            itemsAfterEditing = [...state.tasks, ite]
    }
    ...
    return (
        <section className="todos-list">
            ...
            <div className="todo-list">
                ...
                { state.tasks.length > 0 ? state.tasks.map( (item) => <TodoItem key={item.id} task={item} setItemState={setItemState} /> ) : (<div className="todo"> There are no items, yet! </div>) }
            </div>
